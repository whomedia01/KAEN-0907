      ...
      <footer className="mt-20 hidden border-t border-gray-300 bg-white md:block">
        <div className="border-b bg-slate-100">
          <nav className="mx-auto flex max-w-6xl flex-wrap items-center justify-center px-4 py-3 text-[13px] text-gray-900">
            ...
            <FooterLink href="/correction">정정보도청구</FooterLink><Sep />
            <FooterLink href="/report">기사제보</FooterLink><Sep />
            <FooterLink href="/partnership">광고·제휴문의</FooterLink>
          </nav>
        </div>
        ...
        <div className="relative mx-auto flex max-w-6xl gap-10 px-4 py-7 text-[12px] leading-[1.65] text-gray-600">
          ...
          <div className="w-[170px] shrink-0 pt-2">
            <Link href="/" className="block leading-none text-gray-700">
              <span className="block text-[24px] font-black tracking-[-0.06em] text-slate-800">한국AI교육신문</span>
              <span className="mt-1.5 block text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">KOREA AI EDUCATION NEWS</span>
            </Link>
          </div>

          <div className="min-w-0 flex-1 pr-10">
            <div className="space-y-0.5 break-keep text-gray-600">
              <p>제호 {settings.site_name} | 발행·운영 {settings.business_name ?? settings.operator_name ?? '(주)후미디어'} | 대표자 {settings.representative_name ?? '황광성'}</p>
              <p>발행인·편집인 {settings.publisher_name ?? '황광성'} | 청소년보호책임자·고충처리인 {settings.youth_protection_manager ?? '황광성'} | 개인정보보호책임자 {settings.privacy_manager ?? '황광성'}</p>
              <p>본사 주소 {settings.address ?? '서울특별시 금천구 가산디지털2로 53 (가산동) 한라시그마밸리 1102호 ~ 1104호'}</p>
              <p>대표전화 {settings.contact_phone ?? '02-6443-4222'} | 팩스 {settings.contact_fax ?? '02-6443-4223'} | 공식 이메일 {settings.contact_email ?? 'whomedia03@gmail.com'}</p>
              <p>사업자등록번호 {settings.business_registration_number ?? '119-86-25861'} | 인터넷신문 등록번호 {settings.media_registration_number || '등록 신청 예정'}</p>
