        ...
        <div className="relative space-y-3 font-sans text-left">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-2xl font-bold text-slate-900 tracking-tight font-bareun-batang">
                {settings?.newspaperName || "한국AI교육신문"}
              </h2>
            </div>
          ...
          <div className="text-[12px] text-slate-600 leading-relaxed space-y-1.5 pt-1">
            <div className="flex flex-wrap gap-x-3 gap-y-1 items-center">
              <span><strong>인터넷신문 등록번호:</strong> {settings?.registrationNo || ""}</span>
              {settings?.registrationDate ? (
                <>
                  <span className="text-slate-300">|</span>
                  <span><strong>등록연월일:</strong> {settings.registrationDate}</span>
                </>
              ) : null}
              <span className="text-slate-300">|</span>
              <span><strong>발행인·편집인:</strong> {settings?.representative || "황광성"}</span>
              <span className="text-slate-300">|</span>
              <span><strong>청소년보호책임자:</strong> {settings?.youthOfficer || "황광성"}</span>
              <span className="text-slate-300">|</span>
              <span><strong>고충처리인:</strong> {settings?.grievanceOfficer || "황광성"}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1 items-center">
              ...
              <span className="text-slate-300">|</span>
              <span><strong>사업자등록번호:</strong> {settings?.businessLicenseNo || ""}</span>
            </div>

            <div className="flex flex-wrap gap-x-3 gap-y-1 items-center">
              <span><strong>본사 주소:</strong> {settings?.address || ""}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1 items-center">
              ...
              {settings?.fax ? (
                <>
                  <span className="text-slate-300">|</span>
                  <span><strong>Fax:</strong> {settings.fax}</span>
                </>
              ) : null}
              <span className="text-slate-300">|</span>
              <span><strong>E-mail:</strong> {settings?.email || ""}</span>
            </div>
            ...
            <div className="pt-3 border-t border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center text-[11px] text-slate-500 gap-1.5">
              <p>
                Copyright © {settings?.newspaperName || "한국AI교육신문"} All rights reserved.
              </p>
