...
import { PRESS_IMAGE_DATABASE, matchArticleImage, createInlineFigureHtml } from "./src/data/pressImages";
...
const defaultAuthors = [
  { id: "auth_publisher", name: "황광성", role: "Publisher", email: "editor@kaen-news.kr", avatarUrl: "/media/hwang_kwang_sung.svg", bio: "한국AI교육신문 발행인 · 편집인" },
  { id: "auth_policy", name: "교육정책 취재팀", role: "Reporter", email: "editor@kaen-news.kr", avatarUrl: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=200", bio: "한국AI교육신문 교육정책 취재팀" },
  { id: "auth_edtech", name: "에듀테크 취재팀", role: "Reporter", email: "editor@kaen-news.kr", avatarUrl: "https://images.unsplash.com/photo-1580489944761-15a19d654956?auto=format&fit=crop&q=80&w=200", bio: "한국AI교육신문 에듀테크 산업 취재팀" },
  { id: "auth_editorial", name: "편집국 취재팀", role: "Editor", email: "editor@kaen-news.kr", avatarUrl: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=200", bio: "한국AI교육신문 종합 편집국" }
];
...
const defaultAuditLogs = [
  {
    ...
    userRole: "Admin",
    action: "SYSTEM_INIT",
    details: "한국AI교육신문 CMS 통합 백엔드 DB 가동 완료",
...
const defaultLayoutSettings = [
  ...
  { sectionId: "sec_photo", name: "포토 및 영상 리포트", displayOrder: 4, enabled: true }
];
...
function loadDbData() {
  ...
  if (dbData.articles && Array.isArray(dbData.articles)) {
    ...
    // 기사 ID별 정밀 매핑 테이블 (기사 내용과 100% 일치하며 중복 배제)
    const articleExactImageMap: Record<string, string> = {
      // 1. 초기 25개 주요 기사
      "art_20260801_01": "img_policy_gov_briefing",
      "art_20260801_02": "img_school_concept_95percent",
      "art_20260801_03": "img_edtech_lesson_copilot",
      "art_20260801_04": "img_literacy_digital_divide_safe",
      "art_20260801_05": "img_opinion_role_change_warmth",

      "art_20260802_01": "img_policy_budget_1trillion",
      "art_20260802_02": "img_school_teacher_training_prompt",
      "art_20260802_03": "img_edtech_smart_feedback",
      "art_20260802_04": "img_literacy_keris_guideline",
      "art_20260802_05": "img_opinion_global_field_uk_us",

      "art_20260803_01": "img_policy_cyber_security",
      "art_20260803_02": "img_school_science_lab",
      "art_20260803_03": "img_edtech_verified_seal_50",
      "art_20260803_04": "img_literacy_youth_camp",
      "art_20260803_05": "img_opinion_legal_ethics_mission",

      "art_20260804_01": "img_policy_cloud_datacenter",
      "art_20260804_02": "img_school_adaptive_tutoring",
      "art_20260804_03": "img_edtech_standard_consortium",
      "art_20260804_04": "img_literacy_library_citizen",
      "art_20260804_05": "img_opinion_autonomy_human_touch",

      "art_20260805_01": "img_policy_national_standards",
      "art_20260805_02": "img_school_tablet_classroom",
      "art_20260805_03": "img_edtech_global_export_300m",
      "art_20260805_04": "img_literacy_deepfake_signature",
      "art_20260805_05": "img_opinion_editorial_promise",

      // 2. 자동 발행 및 추가 기사들
      "art_auto_1788755325356": "img_edtech_startup_ir",
      "art_auto_1788755317601": "img_opinion_teacher_designer",
      "art_auto_1788755303743": "img_literacy_senior_education",
      "art_auto_1788755285872": "img_school_smart_board",
      "art_auto_1788755281499": "img_policy_highspeed_network",
      "art_auto_1785980903584": "img_policy_local_board",
      "art_auto_1785976553145": "img_opinion_hwang_column_thought",
      "art_auto_1785976552947": "img_literacy_deepfake_defense",
      "art_auto_1785976552689": "img_edtech_global_summit_booth",
      "art_auto_1785918357464": "img_school_sw_coding",
      "art_auto_1785908061076": "img_policy_mou_agreement"
    };
    ...
    dbData.articles.forEach((a: any) => {
      ...
      let targetItem = null;
      if (articleExactImageMap[a.id]) {
        targetItem = PRESS_IMAGE_DATABASE.find(item => item.id === articleExactImageMap[a.id]);
      }

      // 지정 매핑이 없거나 중복된 URL인 경우 자동 중복 배제 매칭
      if (!targetItem || usedUrls.has(targetItem.url)) {
        targetItem = matchArticleImage(a.title || "", a.content || "", a.categoryId, usedUrls);
      }

      if (targetItem) {
        if (a.imageUrl !== targetItem.url) {
          a.imageUrl = targetItem.url;
          a.imageCaption = targetItem.caption;
          a.imageCopyright = targetItem.copyright;
          hasChanges = true;
        }
        usedUrls.add(targetItem.url);

        // 본문 내 인라인 이미지도 새로운 맞춤 사진으로 동기화
        const inlineFig = createInlineFigureHtml(targetItem);
        if (a.content) {
          if (a.content.includes('<figure')) {
            // 기존 figure 교체
            a.content = a.content.replace(/<figure[\s\S]*?<\/figure>/g, inlineFig.trim());
            hasChanges = true;
          } else if (a.content.includes('</h3>')) {
            const parts = a.content.split('</h3>');
            a.content = parts[0] + '</h3>' + inlineFig + parts.slice(1).join('</h3>');
            hasChanges = true;
          } else if (a.content.includes('</p>')) {
            const parts = a.content.split('</p>');
            a.content = parts[0] + '</p>' + inlineFig + parts.slice(1).join('</p>');
            hasChanges = true;
          }
        }
      }
  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        const prompt = `당신은 대한민국 대표 인공지능·미래교육 전문 언론사 '한국AI교육신문'의 팩트체크 수석 기자인 '황광성 편집인'입니다.
주제: ${topicItem.topic}
...
응답은 오직 아래 JSON 규격으로만 출력하세요:
{
  ...
  "content": "<p><strong>[한국AI교육신문 = 취재팀]</strong> 본문 첫 단락...</p><h3>■ 소제목 1</h3><p>세부 내용...</p><h3>■ 소제목 2</h3><p>세부 내용...</p>",
  "factScore": 98
}`;
    ...
    if (!generatedTitle) {
      ...
      generatedContent = ensureArticleLength(`<p><strong>[한국AI교육신문 = ${topicItem.categoryName} 취재팀]</strong> 정부 및 전국 17개 시·도교육청이 추진하는 미래 교육 디지털 전환 사업의 최신 성과 지표와 현장 실증 결과가 발표됐다.</p>
...
<p>현장 전문가들은 기술 도입에 맞춰 학생 개인정보 보호 및 저작권 준수 가이드라인을 철저히 강화해야 한다고 지적했다.</p>` + (dbData.automationSettings?.includeLegalDisclaimer !== false ? legalDisclaimer : ""));
    ...
    // 팩트 보도사진 정밀 매칭 (기사 제목, 본문, 카테고리 기반 및 중복 배제)
    const existingUrls = new Set<string>(dbData.articles.map((a: any) => a.imageUrl).filter(Boolean));
    const matchedImg = matchArticleImage(generatedTitle, generatedContent, topicItem.categoryId, existingUrls);

    // 본문 내에 인라인 보도사진이 없는 경우 적절한 위치에 정밀 삽입
    if (!generatedContent.includes('<figure') && !generatedContent.includes('<img')) {
      const inlineFigure = createInlineFigureHtml(matchedImg);
      if (generatedContent.includes('</h3>')) {
        const parts = generatedContent.split('</h3>');
        generatedContent = parts[0] + '</h3>' + inlineFigure + parts.slice(1).join('</h3>');
      } else if (generatedContent.includes('</p>')) {
        const parts = generatedContent.split('</p>');
        generatedContent = parts[0] + '</p>' + inlineFigure + parts.slice(1).join('</p>');
      }
    }
    ...
    const newArticle = {
      ...
      imageCaption: matchedImg.caption,
      imageCopyright: matchedImg.copyright,
      status: "published",
      scheduledAt: null,
      createdAt: nowIso,
      viewCount: 1,
...
async function startServer() {
  ...
  // Press Image Archive Library
  app.get("/api/press-images", (req, res) => {
    const { category } = req.query;
    if (category) {
      const filtered = PRESS_IMAGE_DATABASE.filter(img => img.category === category);
      return res.json(filtered);
    }
    res.json(PRESS_IMAGE_DATABASE);
  });

  // Recommend best matching image for given title, content, category
  app.post("/api/articles/recommend-image", (req, res) => {
    const { title, content, categoryId } = req.body;
    const matched = matchArticleImage(title || "", content || "", categoryId);
    res.json(matched);
  });
  ...
  app.post("/api/site-settings/test-sync", (req, res) => {
    res.json({
      status: "success",
      message: "한국AI교육신문 DB 및 외부 연동 서비스 정상 동기화 완료"
    });
  });
...
아래 JSON 형식에 맞추어 전문적이고 신뢰도 높은 한국어 뉴스 기사를 작성해 주세요:
{
  ...
  "content": "<p><strong>[한국AI교육신문 = 취재팀]</strong> 본문 첫 단락...</p><h3>■ 소제목 1</h3><p>상세 본문 단락 1...</p><h3>■ 소제목 2</h3><p>상세 본문 단락 2...</p><h3>■ 소제목 3</h3><p>상세 본문 단락 3...</p>",
  "tags": ["키워드1", "키워드2", "키워드3"],
  "faqList": [
    { "q": "자주 묻는 질문 1", "a": "답변 1" },
    { "q": "자주 묻는 질문 2", "a": "답변 2" }
